...
Vertex FindingMaxDegree(const Graph &data, const Graph &query,
  ...
  // return -1 if there can't be any answer
  if (is_answer == false) return -1;

  return v;
}

std::vector<Vertex> FindingMatchingOrder(Vertex v, const Graph &data, const Graph &query,
                                const CandidateSet &cs) {
  /**
...
void Backtracking(std::vector<Vertex> matchingOrder, const Graph &data, const Graph &query,
                                const CandidateSet &cs) {
...
void Backtrack::PrintAllMatches(const Graph &data, const Graph &query,
  ...
  Vertex v = FindingMaxDegree(data, query, cs);
  if (v==-1) return;

  //finding out matching order
  std::vector<Vertex> matchingOrder = FindingMatchingOrder(v, data, query, cs);
  
  //backtracking and printing out
  Backtracking(matchingOrder, data, query, cs);
